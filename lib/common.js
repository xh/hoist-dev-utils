...
/**
 * Bundler-agnostic helpers behind `configureRsbuild()`: app / library version resolution,
 * entry-point discovery, CHANGELOG parsing, Blueprint icon stubs, manifest content, and the console
 * banner. Nothing in here touches a bundler API - anything that does belongs in
 * `configureRsbuild.js` or a plugin under `lib/`.
 */
...
// v88 is the TC39-decorators release: this build emits the 2023-11 transform, under which an
// earlier hoist-react's legacy `@observable` / `@bindable` fields silently stop working.
...
/**
 * Every `env` option `configureRsbuild()` understands. Keep in step with its JSDoc. Options it
 * rejects outright (`rejectUnsupported()` there) are deliberately absent.
 */
const KNOWN_OPTIONS = [
...
/**
 * Warn about `env` keys the config does not recognize. A misspelled or long-removed option is otherwise
