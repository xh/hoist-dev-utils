...
// Higher floor for `configureRsbuild()` in `decoratorTransform: 'swc'` mode only. SWC emits
// TypeScript-style legacy decorators, under which field decorators receive no descriptor -
// hoist-react's `@persist` relies on Babel's `descriptor.initializer` and fails at startup with a
// TypeError under that emit. Reserved for hoist-react releases whose decorators are
// transpiler-agnostic (the TC39 decorators migration, targeted at v88). The default 'babel' mode
// shares MIN_HOIST_REACT_VERSION with configureWebpack().
const MIN_HOIST_REACT_VERSION_RSBUILD = '88.0';
 ...
 */
function checkHoistReactVersion(hoistReactPkg, inlineHoist, minVersion = MIN_HOIST_REACT_VERSION) {
    ...
    if (
        ...
        throw (
            ...
            `${minVersion} - found v${hoistReactPkg.version}. Upgrade @xh/hoist, ` +
