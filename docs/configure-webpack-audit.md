...
| Item | Bin | Notes |
|---|---|---|
| `@babel/preset-react` / JSX support generally | 🟢 | JSX stays a standard capability - some clients insist on it - but **only via `.tsx`**: per ATM (Aug 2026), any app supported by new releases must be TS, so `.tsx` is the sole JSX carrier going forward. Free in every candidate toolchain. Optional modernization: `runtime: 'automatic'` (jsx-runtime) instead of classic `createElement`. |
...
| Polyfill layer: `useBuiltIns: 'entry'` + `corejs: {version 3, proposals: true}` + prepending hoist's `static/polyfills.js` to every entry + app-level `core-js` dep | 🟡 - measured + with history | **Measured (Aug 2026, this package's own Babel, current default targets): the rewrite injects 77 modules, of which zero are features any target browser lacks.** ~75 are `esnext.*` shims for mostly *dead or stale proposals* (Observable, Reflect metadata, seeded PRNG, `Math.iaddh`, pattern-match, old Map/Set method proposals...) - pulled in because `polyfills.js` imports the `core-js` *root* entry, which includes proposals regardless of the `proposals` option (that option only affects `useBuiltIns: 'usage'`). The other two: `web.immediate` (`setImmediate`, never standardized) and `es.array.includes` (a compat-data quirk - native everywhere since 2016). So for evergreen-targeted apps the layer currently ships pure dead weight; its real function is insurance that activates only when an app widens `targetBrowsers` (legacy-webview case). **History**: removal was tried in hoist-react #4334 (2026-04-17) and reverted same-day in #4339 - precautionary, no documented runtime breakage; the removal had also bypassed this package's side of the mechanism. **Legacy-webview constraint appears to be retiring** (clients moving to modern/Chromium-based managed browsers) - confirm the current client browser matrix before final removal. **Low-risk immediate step: change `polyfills.js` to `import 'core-js/stable'`** - drops all ~75 proposal shims for every app while keeping the auto-scaling insurance mechanism intact; full removal then becomes a later, coordinated two-repo change once the browser matrix is confirmed. |
...
| `@babel/plugin-transform-typescript` configured explicitly alongside `@babel/preset-typescript` (`isTSX: true`, `allowDeclareFields`) | 🔴 | Duplication - the preset wraps the same plugin - and the explicit plugin's whole purpose (`isTSX: true` = JSX inside plain `.js` files, for pre-TS-era apps) is no longer a supported configuration: new releases are TS-only, JSX only via `.tsx`. Collapse to a single preset/parser config; do not port an opt-in flag. |
...
| `resolve.extensions` including `.jsx` | 🔴 | Drop - new releases support TS apps only, with JSX carried solely by `.tsx`. Zero `.jsx` files exist in the ecosystem today. Plain `.js` stays in the loader test/extensions (hoist's `static/polyfills.js` entry and any stray JS must still transpile), just without JSX parsing. |
