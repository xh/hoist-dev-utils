...
2. **dev-utils (webpack, minor/patch)**: delete the two 🔴 forced Babel includes, the stale
   ...
   Run the preset-env `debug: true` polyfill audit and, in all likelihood, remove the polyfill
   layer (coordinated with removing `static/polyfills.js` from hoist-react and `core-js` from
   apps - small breaking note).
